...
peakcallingGPU18_short3.py input output1 output2 fdr p w

input is a text file with one line per chromosome, each line in the format:
chr_number   observed_binary_matrix   expected_vector    norm_vector   resolution

output1 is the name of the output file that has the fdr thresholds printed to it

output2 is the name of the output file that has the enriched pixels printed to it

fdr is an int corresponding to 1 over the maximum acceptable q-value (i.e. 10% FDR means 10, 1% FDR would be 100, etc)

p is an int corresponding to the peak width

w is an int corresponding to the minimum window size

i'd test on an input file with only one chromosome at first. let me know if you have questions.
when you get to the listgiven script, let's talk, that one is really janky because i was lazy so it shouldn't be ported as is. 
"""
		...
		for i in xrange((dim/500)+1):
			...
			for j in xrange(i,(dim/500)+1):
				...
				if run==1:
					...
					nopeaks=len(index_dense_cpu[peak_index])	
					...
					for k in xrange(nopeaks):
						...
						if int(bpbl[k])<40 and op[k]<10000 and int(bpdonut[k])<40 and int(bph[k])<40 and int(bpv[k])<40:
							...
							outfile2.write(input_splits[0]+"\t"+str(row*res)+"\t"+input_splits[0]+"\t"+str(col*res)+"\t"+str(op[k])+"\t"+str(epbl[k])+"\t"+str(epdonut[k])+"\t"+str(eph[k])+"\t"+str(epv[k])+"\t"+str(bpbl[k])+"\t"+str(bpdonut[k])+"\t"+str(bph[k])+"\t"+str(bpv[k])+"\t"+str(fdr_bl)+"\t"+str(fdr_donut)+"\t"+str(fdr_h)+"\t"+str(fdr_v)+"\n")
		if run==0:
			hist_time=time.time()
			...
			if sum(hist_bl[i,:])>0:
				for j in xrange(10000):
					if fdr*sum(expectedbl[j:10000])<=sum(hist_bl[i,j:10000]):
						...
						break
				for j in xrange(threshold_bl[i],10000):
			...
			if sum(hist_donut[i,:])>0:
				for j in xrange(10000):
					if fdr*sum(expecteddonut[j:10000])<=sum(hist_donut[i,j:10000]):
						...
						break
				for j in xrange(threshold_donut[i],10000):
			...
			if sum(hist_h[i,:])>0:
				for j in xrange(10000):
					if fdr*sum(expectedh[j:10000])<=sum(hist_h[i,j:10000]):
						...
						break
				for j in xrange(threshold_h[i],10000):
			...
			if sum(hist_v[i,:])>0:
				for j in xrange(10000):
					if fdr*sum(expectedv[j:10000])<=sum(hist_v[i,j:10000]):
						...
						break
				for j in xrange(threshold_v[i],10000):
